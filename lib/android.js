...
class TextControl {
  constructor() {
    this._shadowPages = new Map();
    this._shadowPool = {
      chunks: [],
      current: null,
    };
    // pageKey -> Set<pcString>: 每个受保护页对应的所有已注册蹦床 PC
    this._protectedPages = new Map();
    this.useMemoryProtect = true;
    this.useTextShadowProtect = true;

    // 异步合批: writeTextProtect 调用会入队, 5ms 内聚合同段/同页, 一次 rwx 窗口
    // + 一次 CModule 批量 prctl(PROTECT). 同页迟到 hook 仍走 unprotect -> rwx ->
    // CModule ts_protect_batch 的原子序列, 白名单 gap 压到 μs 级.
    this._queue = [];
    this._flushTimer = null;
    this._flushDelayMs = 5;
    this._icacheLine = 64;   // ARM64 常见 cacheline, 刷指令 cache 的对齐粒度
  ...
  _initFunctions() {
    ...
    const mmapPtr = Module.findExportByName(libc, 'mmap');
    if (mmapPtr === null) throw new Error('[ts] mmap not found in libc.so');
    this._mmap = new NativeFunction(mmapPtr, 'pointer', ['pointer', 'ulong', 'int', 'int', 'int', 'int64']);

    const mprotectPtr = Module.findExportByName(libc, 'mprotect');
    if (mprotectPtr !== null) {
      this._mprotect = new NativeFunction(mprotectPtr, 'int', ['pointer', 'ulong', 'int']);
    }

    this._prctlPtr = Module.findExportByName(libc, 'prctl');
    if (this._prctlPtr === null) throw new Error('[ts] prctl not found in libc.so');
    this._prctl = new NativeFunction(this._prctlPtr, 'int', ['int', 'uint64', 'uint64', 'uint64', 'uint64']);

    const munmapPtr = Module.findExportByName(libc, 'munmap');
    if (munmapPtr !== null) {
  ...
  _initCModule() {
    // 用 CModule 把 prctl(PROTECT) 的批量循环放到 native, 避免 V8<->C 边界
    // 和解释器开销, 把同页 append 的 N 次 syscall 压到一个直线 C 循环里.
    try {
      const src = `
...
/* pcs: uint64[] n, 每个元素是一个蹦床入口 PC.
 * orig: 该页 saveOriginal 分配的 clean 页缓冲区地址.
 * kernel protect_page: 首调 PC 分配 alt 并从 orig 拷贝, 后续同页 PC 走 append. */
...
/* 同页已保护场景: 一次 syscall 解保护后立即批量重建, 白名单 gap 尽量短. */
...
`;
      ...
      console.log('[ts] CModule init failed, fallback to JS loop:', e && e.message ? e.message : e);
      this._cm = null;
    }
  }
  ...
  _mapShadowChunk(minSize) {
    ...
    const chunkBase = this._mmap(ptr(0), chunkSize, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  ...
  saveOriginal(addr) {
    ...
    if (this._shadowPages.has(key)) {
      return this._shadowPages.get(key).original;
    }
    const buf = this._allocShadowBytes(PAGE_SIZE);
    if (this._memcpy !== undefined) {
      this._memcpy(buf, base, PAGE_SIZE);
    } else {
      Memory.copy(buf, base, PAGE_SIZE);
    }
    this._shadowPages.set(key, { original: buf });
    return buf;
  }

  // 只做一次 prctl 调用; 不维护 _protectedPages (由 caller 管理, 以便区分 append 场景)
  _prctlProtect(func) {
  ...
  protect(func) {
    ...
    if (ret === 0 || ret > 0) {
      ...
      if (!set) {
        set = new Set();
        this._protectedPages.set(pageKey, set);
      }
      set.add(pcKey);
    }
    return ret;
  ...
  writeTextProtect(address, callback) {
    ...
    // Phase 0: 原件必须同步保存 (同页不同 hook 的 saveOriginal 互不冲突, 做在入队前
    // 避免 rwx 窗口开后 page 已是 hook 字节时再去拷贝).
  ...
  _scheduleFlush() {
    ...
    this._flushTimer = setTimeout(() => {
      ...
      try {
        ...
        console.log('[ts] flush error:', (e && e.stack) || e);
      }
    }, this._flushDelayMs);
  }

  // 强制立即落盘 (初始化末尾 / 测试同步期望时可主动调)
  flush() {
  ...
  _flush() {
    ...
    // 1) 按 page 分组
    const byPage = new Map(); // pageKey -> [items]
    ...
    // 2) 搜集所有涉及的 range, 一次 rwx 窗口覆盖整个 batch
    ...
    // 3) 针对已保护页 (同页后续 hook), 先 unprotect 让接下来的 patch 落进 shadow.
    //    必须在 rwx 前做, 否则 mprotect_before 内核钩子会把 PFN 预翻成 alt,
    //    Frida 的写动作会污染 alt.
    ...
    // 5) 每个 page 一次 CModule 批量 protect (新 PC + 保留 PC 合并),
    //    压缩 prctl 循环的 JS 开销, 并把白名单 gap 收窄到 μs 级.
    for (const [pk, info] of perPage) {
      ...
      // 6) 保护之后再按 addr ±cacheline 做局部 icache 失效, 避免整段 .so 遍历
...
function instrumentArtMethodInvocationFromInterpreter () {
  ...
  for (const exp of Module.enumerateExports('libart.so').filter(exp => artInterpreterDoCallExportRegex.test(exp.name))) {
    tc.writeTextProtect(exp.address, () => {
...
function ensureArtKnowsHowToHandleReplacementMethods (vm) {
  ...
  if (mayUseCollector(kCollectorTypeCMC)) {
    ...
    tc.writeTextProtect(target, () => {
      Interceptor.attach(target, artController.hooks.Gc.runFlip);
    });
  } else {
    ...
    if (copyingPhase !== null) {
      tc.writeTextProtect(copyingPhase, () => {
...
class JdwpSession {
  constructor () {
    ...
    // tc.writeTextProtect(acceptImpl, () => {
...
function fixupArtQuickDeliverExceptionBug (api) {
   ...
   */
  const tc = getTextControl();
  // tc.writeTextProtect(prettyMethod.impl, () => {
